...
describe('scan-manager edge cases', () => {
  it('handles directory morphing into file mid-scan, detects morph file node, may truncate early', async () => {
    ...
    const { scanId } = scanManager.startScan(tmp, { batchSize: 10, maxEntries: 15 });
    ...
    const morphNode = allNodes.find(n => n.path === changingDir && n.depth === 1 && n.kind === 'file');
    expect(morphNode).toBeTruthy();
