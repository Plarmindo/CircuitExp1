...
describe('VIS-18 coverage extended', () => {
  ...
  it('adapter metadata update branches (size, mtime, error) and no-op unchanged delta', () => {
    ...
    adapter.applyDelta([
      ...
      { path: '/root/file.txt', parentPath: '/root', kind: 'file', name: 'file.txt', depth: 1, sizeBytes: 10, mtimeMs: 100 }
    ]);
    ...
    const r2 = adapter.applyDelta([
      { path: '/root/file.txt', parentPath: '/root', kind: 'file', name: 'file.txt', depth: 1, sizeBytes: 20, mtimeMs: 105, error: 'E' }
    ]);
  ...
  it('ensureParents partial existing chain branch & deeper placeholder addition', () => {
    ...
    const r = adapter.applyDelta([
      { path: '/root/a/b/file.txt', parentPath: '/root/a/b', kind: 'file', name: 'file.txt', depth: 3 }
    ]);
  ...
  it('layout aggregated expanded recursion places grandchildren (branch coverage) and coordinate stability across runs', () => {
    ...
    interface DirNode { path: string; parentPath: string; kind: 'dir' | 'file'; name: string; depth: number }
    const dirs: DirNode[] = [];
    for (let i=0;i<6;i++) { // > aggregationThreshold we will set (3) to force aggregation
      dirs.push({ path: `/root/d${i}`, parentPath: '/root', kind: 'dir', name: `d${i}`, depth: 1 });
      dirs.push({ path: `/root/d${i}/f.txt`, parentPath: `/root/d${i}`, kind: 'file', name: 'f.txt', depth: 2 });
    }
    ...
    const synthetic = firstCollapsed.nodes.find(n => (n as { aggregated?: boolean }).aggregated)!.path;
    expanded.add(synthetic);
    const l1 = layoutHierarchicalV2(adapter, { aggregationThreshold: 3, expandedAggregations: expanded });
    // Expect grandchildren (files depth 2) to be present
    const depth2 = l1.nodes.filter(n => n.depth === 2 && !(n as { aggregated?: boolean }).aggregated);
    ...
    const l2 = layoutHierarchicalV2(adapter, { aggregationThreshold: 3, expandedAggregations: expanded });
