...
describe('PERF-1 fast path integration (jsdom)', () => {
  it('pure append tail batch increments fastPathUses without new layoutCallCount', async () => {
    ...
    const getDbg = (): MetroDebug | undefined => (window as unknown as { __metroDebug?: MetroDebug }).__metroDebug;
  ...
  const dbgAny = dbg as unknown as { genTree?: (b: number, d: number, f: number) => number; getNodes: () => Array<{ path: string }> };
