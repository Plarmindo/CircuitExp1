...
describe('VIS-19 incremental consistency', () => {
  it('final layout matches single-pass layout ignoring aggregation placeholders', () => {
    ...
    const baseIndex = new Map(baselineLayout.nodes.map(n => [n.path, { x: n.x, y: n.y, depth: n.depth }]));
  ...
  it('final layout matches with aggregation active (small threshold)', () => {
    ...
    const baselineLayout = layoutHierarchicalV2(baselineAdapter, { aggregationThreshold: aggThreshold });
  ...
  interface AggLike { aggregated?: boolean; aggregatedCount?: number }
  const isAgg = (o: unknown): o is AggLike => !!o && typeof o === 'object' && 'aggregated' in (o as Record<string, unknown>);
