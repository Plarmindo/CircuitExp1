...
test('basic UI flow: generate tree, zoom, select, export PNG (>1KB)', async ({ page }) => {
  ...
  await page.waitForFunction(() => (window as unknown as { __metroDebug?: unknown }).__metroDebug !== undefined, { timeout: 10000 });
  await page.waitForFunction(() => (window as unknown as { __metroDebug?: { genTree?: unknown } }).__metroDebug?.genTree, { timeout: 10000 });
  await page.evaluate(() => {
  ...
  await page.evaluate(() => { window.dispatchEvent(new Event('metro:zoomIn')); });
  await page.waitForTimeout(120);
  await page.evaluate(() => { window.dispatchEvent(new Event('metro:zoomOut')); });
  await page.waitForTimeout(120);
  ...
  const picked = await page.evaluate(() => {
    ...
    if (first) {
      ...
      if (canvas && first.clientX && first.clientY) {
        const evtPtr = new PointerEvent('pointerdown', { clientX: first.clientX, clientY: first.clientY });
        canvas.dispatchEvent(evtPtr);
        const evtTap = new PointerEvent('pointerup', { clientX: first.clientX, clientY: first.clientY });
  ...
  await page.evaluate(() => {
    ...
    if (btn) btn.click(); else window.dispatchEvent(new CustomEvent('metro:exportPng'));
  });
  await page.waitForFunction(() => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
