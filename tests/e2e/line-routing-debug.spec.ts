...
interface DebugRouteCommand { type: 'M'|'L'|'Q'; x?: number; y?: number; cx?: number; cy?: number }
interface DebugRoute { key: string; commands: DebugRouteCommand[] }
interface DebugApi { lastRoutes?: DebugRoute[] }
...
test.describe('VIS-23 line routing debug verification', () => {
  test('emits at least one Q curve and avoids long raw diagonals', async ({ page }) => {
    await page.waitForFunction(() => !!(window as unknown as { __metroDebug?: DebugApi }).__metroDebug);
    // Generate a reasonably sized tree
    await page.evaluate(() => {
      (window as unknown as { dispatchEvent: (e: Event) => void }).dispatchEvent(new CustomEvent('metro:genTree', { detail: { breadth: 6, depth: 3, files: 4 } }));
    });
    ...
    const routeInfo = await page.evaluate(async () => {
      ...
      for (let i=0;i<5;i++) {
        (window as unknown as { dispatchEvent: (e: Event) => void }).dispatchEvent(new CustomEvent('metro:themeChanged'));
