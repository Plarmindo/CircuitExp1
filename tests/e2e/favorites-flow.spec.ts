...
test.describe('CORE-1 favorites flow', () => {
  test('add favorite + jump centers node', async ({ page }) => {
    ...
    await page.evaluate(() => { // @ts-expect-error accessing injected debug helper
      if (window.__metroDebug?.genTree) window.__metroDebug.genTree(2,2,1); else window.dispatchEvent(new CustomEvent('metro:genTree', { detail: { breadth: 2, depth: 2, files: 1 } }));
    });
    ...
    await page.evaluate((p) => window.dispatchEvent(new CustomEvent('metro:select', { detail: { path: p, type: 'node' } })), first.path);
    ...
    await expect(page.locator('.favorites-list .fav-item .fav-jump', { hasText: base })).toHaveCount(1);
  ...
  await expect(page.locator('.selected-section .value.path', { hasText: first.path })).toBeVisible();
  });
});
