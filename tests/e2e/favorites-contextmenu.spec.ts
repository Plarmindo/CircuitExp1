...
test.describe('CORE-1 favorites context menu', () => {
  test('right-click add & remove favorite', async ({ page }) => {
    ...
    await page.evaluate(() => { // @ts-expect-error accessing injected debug helper
      if (window.__metroDebug?.genTree) window.__metroDebug.genTree(3,2,2); else window.dispatchEvent(new CustomEvent('metro:genTree', { detail: { breadth: 3, depth: 2, files: 2 } }));
    });
    ...
    await page.evaluate((p) => {
      window.dispatchEvent(new CustomEvent('metro:contextMenu', { detail: { path: p, x: 200, y: 200 } }));
    }, node.path);
  ...
  await page.locator(menuSelector + ' button').first().click();
  // Favorite should appear in list (allow some retries)
  await expect(page.locator('.favorites-list .fav-item .fav-jump', { hasText: base })).toHaveCount(1, { timeout: 10000 });

    // Open context menu again for same node (simulate right click again)
    await page.evaluate((p) => {
      window.dispatchEvent(new CustomEvent('metro:contextMenu', { detail: { path: p, x: 210, y: 210 } }));
    }, node.path);
  await expect(page.locator(menuSelector)).toBeVisible();
  // Remove favorite (same single button toggles)
  await page.locator(menuSelector + ' button').first().click();
  await expect(page.locator('.favorites-list .fav-item .fav-jump', { hasText: base })).toHaveCount(0, { timeout: 10000 });
  });
});
