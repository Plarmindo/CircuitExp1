...
export function performFastAppend(opts: FastAppendOptions) {
  ...
  adapter.applyDelta(newNodes as Array<{ path: string; name: string; kind: 'dir'; depth: number }>);
  ...
  if (fast) {
    incrementFastPath();
    updateState(fast.nodes as LayoutPointV2[], fast.index as Map<string, LayoutPointV2>);
