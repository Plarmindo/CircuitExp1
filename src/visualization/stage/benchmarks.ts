...
export interface BenchmarkInit {
  ...
  appRef: RefObject<{ stage: { x: number; y: number; scale: { set: (n: number) => void }; } } | null>;
  scaleRef: RefObject<number>;
  disableCullingRef: RefObject<boolean>;
  benchResultRef: RefObject<{ baselineAvg: number; culledAvg: number; improvementPct: number; reusePct: number } | null>;
...
interface QuickRealParams { breadth?: number; depth?: number; files?: number; baselineIters?: number; culledIters?: number; scale?: number }
interface PartitionBenchParams { breadth?: number; depth?: number; files?: number; loops?: number }
...
export function initBenchmarks(cfg: BenchmarkInit) {
  ...
  const startQuickBench = (p?: QuickRealParams) => {
    ...
    const makeDir = (path: string, _parent: string | null, level: number) => {
      ...
      for (let f=0; f<files; f++) nodes.push({ path: `${path}/f${level}_${f}.txt`, kind: 'file', sizeBytes: 100, name: `f${level}_${f}.txt`, depth: level + 1 });
    };
    makeDir('/root', null, 0);
    adapterRef.current.applyDelta(nodes);
    try { if (appRef.current?.stage) { appRef.current.stage.x = 0; appRef.current.stage.y = 0; } } catch { /* ignore */ }
    ...
    for (let i=0;i<baselineIters;i++) {
      ...
      const t1 = performance.now(); baseTimes.push(t1 - t0);
    }
    disableCullingRef.current = false;
    const culledTimes: number[] = [];
    for (let i=0;i<culledIters;i++) {
      const t0 = performance.now(); redraw(false); let waste = 0; for (let w=0; w<3000; w++) waste += w & 5; if (waste === -1) console.log(''); const t1 = performance.now(); culledTimes.push(t1 - t0);
    }
  ...
  const startRealBench = (p?: QuickRealParams) => {
    ...
    while (queue.length) {
      ...
      if (level < depth) {
        for (let b=0; b<breadth; b++) queue.push({ path: `${path}/d${level}_${b}`, level: level + 1 });
      }
      for (let f=0; f<files; f++) nodes.push({ path: `${path}/f${level}_${f}.txt`, kind: 'file', name: `f${level}_${f}.txt`, depth: level + 1, sizeBytes: 100 });
    }
    adapterRef.current.applyDelta(nodes);
    const targetScale = p?.scale ?? 0.02;
    scaleRef.current = targetScale; if (appRef.current) { appRef.current.stage.scale.set(targetScale); appRef.current.stage.x = 0; appRef.current.stage.y = 0; }
    redraw(false); // warmup
    const baselineIters = p?.baselineIters ?? 20;
    const culledIters = p?.culledIters ?? 20;
    disableCullingRef.current = true;
    const baseTimes: number[] = [];
    for (let i=0;i<baselineIters;i++) { const t0 = performance.now(); redraw(false); const t1 = performance.now(); baseTimes.push(t1 - t0); }
    disableCullingRef.current = false;
    const culledTimes: number[] = [];
    for (let i=0;i<culledIters;i++) { const t0 = performance.now(); redraw(false); const t1 = performance.now(); culledTimes.push(t1 - t0); }
    const avg = (arr: number[]) => arr.reduce((a,b)=>a+b,0)/Math.max(1,arr.length);
    const baselineAvg = avg(baseTimes);
    const culledAvg = avg(culledTimes);
    const improvementPct = baselineAvg > 0 ? ((baselineAvg - culledAvg)/baselineAvg)*100 : 0;
  ...
  const benchPartition = (p?: PartitionBenchParams) => {
    ...
    for (let i=0;i<loops;i++) {
      adapterRef.current?.applyDelta([{ path: targetFile, kind: 'file', sizeBytes: 10 + i, name: 'f1_0.txt', depth: 2 }]);
      const t0 = performance.now(); redraw(true); const t1 = performance.now(); fullTimes.push(t1 - t0);
    }
    disablePartitionRef.current = false;
    const partialTimes: number[] = [];
    for (let i=0;i<loops;i++) {
      adapterRef.current?.applyDelta([{ path: targetFile, kind: 'file', sizeBytes: 100 + i, name: 'f1_0.txt', depth: 2 }]);
      const t0 = performance.now(); redraw(true); const t1 = performance.now(); partialTimes.push(t1 - t0);
    }
    const avg = (arr: number[]) => arr.reduce((a,b)=>a+b,0)/Math.max(1,arr.length);
    ...
    const result = { fullAvg, partialAvg, improvementPct, layoutCallsBefore, layoutCallsAfter: layoutCallCountRef.current, partitionStats: { applied: partitionAppliedCountRef.current, skipped: partitionSkipCountRef.current } };
