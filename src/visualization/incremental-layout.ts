...
export interface IncrementalAppendInput {
  ...
  options: { horizontalSpacing: number; spacingThreshold: number; spacingGrowthRate: number; maxSpacingFactor: number; aggregationThreshold: number; verticalSpacing: number };
...
function effectiveSpacing(count: number, base: number, opts: IncrementalAppendInput['options']): number {
...
export function tryIncrementalAppend(input: IncrementalAppendInput): IncrementalAppendResult | null {
  ...
  if (!addedPaths.length) { debug?.('bail:no-added-paths'); return null; }
  ...
  if (parentPath && !parentNode) { debug?.('bail:missing-parent', { parentPath }); return null; }
  for (const p of addedPaths) {
    const n = adapter.getNode(p);
    if (!n) { debug?.('bail:missing-node', { path: p }); return null; }
    if (n.parentPath !== parentPath) { debug?.('bail:parent-mismatch', { path: p, expected: parentPath, actual: n.parentPath }); return null; }
    if (n.children.length) { debug?.('bail:has-children', { path: p }); return null; } // new node already has children -> bail
  }
  // Determine previous sibling set (before addition). We reconstruct by taking current children minus added.
  const allChildren = parentPath ? parentNode!.children.slice() : adapter.getAllNodes().filter(n=>!n.parentPath).map(n=>n.path);
  ...
  if (parentPath) {
    ...
    if (!parentLayoutPoint) { debug?.('bail:parent-not-in-previous-layout', { parentPath }); return null; }
    const parentIndex = previousNodes.findIndex(n => n.path === parentPath);
    if (parentIndex === -1) { debug?.('bail:parent-index-negative', { parentPath }); return null; }
  ...
  if (!subtreeTailOk) { debug?.('bail:not-tail-subtree', { parentPath }); return null; }
  const newTotal = prevSiblingPaths.length + addedPaths.length;
  if (newTotal > options.aggregationThreshold) { debug?.('bail:aggregation-threshold', { newTotal, threshold: options.aggregationThreshold }); return null; } // would aggregate -> must full recompute
  const spacingBefore = effectiveSpacing(prevSiblingPaths.length, options.horizontalSpacing, options);
  const spacingAfter = effectiveSpacing(newTotal, options.horizontalSpacing, options);
  if (spacingBefore !== spacingAfter) { debug?.('bail:spacing-shift', { before: spacingBefore, after: spacingAfter }); return null; } // spacing shift would reposition existing siblings
  // Sort siblings to ensure comparator order maintained and new nodes appended
  const sortedPrev = prevSiblingPaths.slice().sort((a,b)=>siblingComparator(adapter.getNode(a)!, adapter.getNode(b)!));
  const sortedAdded = addedPaths.slice().sort((a,b)=>siblingComparator(adapter.getNode(a)!, adapter.getNode(b)!));
  ...
  if (lastPrev) {
    ...
    for (const ap of sortedAdded) {
      ...
      if (siblingComparator(lastNode, an) > 0) { debug?.('bail:ordering-would-insert', { lastPrev, offending: ap }); return null; } // needs insertion
    }
  }
