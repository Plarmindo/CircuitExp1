...
export interface LayoutPoint { path: string; x: number; y: number; depth: number; }
export interface LayoutBBox { minX: number; minY: number; maxX: number; maxY: number; width: number; height: number; }
export interface LayoutResult { nodes: LayoutPoint[]; bbox: LayoutBBox; nodeIndex: Map<string, LayoutPoint>; }

export interface LayoutOptionsV1 { horizontalSpacing?: number; verticalSpacing?: number; }
const DEFAULTS: Required<LayoutOptionsV1> = { horizontalSpacing: 140, verticalSpacing: 90 };

export function layoutHierarchical(adapter: GraphAdapter, opts: LayoutOptionsV1 = {}): LayoutResult {
  ...
  const bbox: LayoutBBox = { minX, minY, maxX, maxY, width: (maxX - minX) + horizontalSpacing, height: (maxY - minY) + verticalSpacing };
