...
export interface KeyNavContext {
  ...
  layoutIndexRef: React.MutableRefObject<
    Map<
      string,
      {
        x: number;
  ...
  toggleAggregation: (args: {
    aggregatedPath: string;
    childPaths: string[];
    expandedBefore: boolean;
    currentSelection: string | null;
  }) => { expandedAfter: boolean; newSelection: string | null };
}

export function createKeyHandler(ctx: KeyNavContext) {
  const { selectedKeyRef, layoutIndexRef, expandedAggregationsRef, redraw, toggleAggregation } =
    ctx;
  return (e: KeyboardEvent) => {
    const ae = document.activeElement as HTMLElement | null;
    if (ae && (ae.tagName === 'INPUT' || ae.tagName === 'TEXTAREA' || ae.isContentEditable)) return;
    const dirKeyMap: Record<string, 'up' | 'down' | 'left' | 'right'> = {
      ArrowUp: 'up',
      ArrowDown: 'down',
      ArrowLeft: 'left',
      ArrowRight: 'right',
    ...
    if (e.key in dirKeyMap) {
      ...
      for (const [p, v] of layoutIndexRef.current.entries())
        entries.push({ path: p, x: v.x, y: v.y, aggregated: v.aggregated });
      ...
      if (next && next !== selectedKeyRef.current) {
        ...
        window.dispatchEvent(
          new CustomEvent('metro:select', {
            detail: { path: next, type: info?.aggregated ? 'aggregated' : 'node' },
          })
        );
    ...
    if (e.key === 'Enter') {
      ...
      if (sel) {
        ...
        if (info?.aggregated && !expandedAggregationsRef.current.has(sel)) {
          ...
          window.dispatchEvent(
            new CustomEvent('metro:select', {
              detail: selectedKeyRef.current
                ? { path: selectedKeyRef.current, type: 'aggregated' }
                : null,
            })
          );
          redraw(false);
