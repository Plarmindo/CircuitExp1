...
export interface NavNodeEntry { path: string; x: number; y: number; aggregated?: boolean }
 ...
 */
export function findNextDirectional(currentPath: string | null, direction: NavDirection, entries: NavNodeEntry[]): string | null {
  if (!entries.length) return null;
  const dirVec = direction === 'up' ? { x: 0, y: -1 } : direction === 'down' ? { x: 0, y: 1 } : direction === 'left' ? { x: -1, y: 0 } : { x: 1, y: 0 };
  ...
  for (const e of entries) {
    ...
    const dist = len;
    if (!best || dist < best.dist - 1e-6 || (Math.abs(dist - best.dist) < 1e-6 && (cos > best.cos + 1e-6 || (Math.abs(cos - best.cos) < 1e-6 && e.path < best.path)))) {
